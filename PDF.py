...
async def pdfseimg(event):
    ...
    if not msg:
        ...
        for num in range(pdf.numPages):
            ...
            fil = os.path.join(f"pdf/kazu{num + 1}.png")
            ok.append(fil)
        ...
        for o in range(ok):
            ...
            with open(os.path.join("kazu.png"), "wb") as f:
                pw.write(f)
            await event.reply(
                file="kazu.png",
            )
        ...
        os.remove(pdfp)
        try:
            await event.reply(file="kazu.png")
        except PhotoSaveFileInvalidError:
            await event.reply(file="kazu.png", force_document=True)
        os.remove("kazu.png")
...
async def imgscan(event):
    ...
    if not (
        ...
        return
    kazu = await ok.download_media()
    xx = await event.eor(get_string("com_1"))
    image = cv2.imread(kazu)
    ...
    scann = f"Scanned {kazu.split('.')[0]}.pdf"
    im1.save(scann)
...
async def savepdf(event):
    ...
    if not (ok and (ok.media)):
        ...
        return
    kazu = await ok.download_media()
    if kazu.endswith(("png", "jpg", "jpeg", "webp")):
        xx = await event.eor(get_string("com_1"))
        image = cv2.imread(kazu)
        original_image = image.copy()
        ratio = image.shape[0] / 500.0
